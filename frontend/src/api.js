...
// ====================== CORS — OBRIGATÓRIO PARA A RENDER ======================
app.use(
  cors({
    origin: "*",
...
// Preflight CORS para DELETE / PUT
app.options("*", cors());
...
// Criar admin padrão
const adminExists = db.prepare("SELECT * FROM users WHERE email = ?").get("admin@loja.com");
...
app.post(
  ...
  (req, res) => {
    ...
    // Render usa HTTPS e domínio fixo
    const baseUrl = process.env.RENDER_EXTERNAL_URL || `https://ecommerce-loja.onrender.com`;
...
app.post("/api/orders", authMiddleware, (req, res) => {
  ...
  const count = db.prepare("SELECT COUNT(*) AS count FROM orders WHERE order_date = ?").get(today).count;

  const displayNumber = count + 1;

  const orderId = db.prepare(
    "INSERT INTO orders (user_id, total_price, display_number, order_date) VALUES (?, ?, ?, ?)"
  ).run(req.user.id, total_price, displayNumber, today).lastInsertRowid;
...
app.get("/api/orders/my", authMiddleware, (req, res) => {
  const rows = db.prepare("SELECT * FROM orders WHERE user_id = ?").all(req.user.id);
  res.json(rows);
});

// LISTAR PEDIDOS COMPLETOS (ADMIN)
...
// CONFIRMAR PAGAMENTO
app.post("/api/orders/:display/pay", authMiddleware, (req, res) => {
  const displayNumber = req.params.display;
  const today = new Date().toISOString().slice(0, 10);

  const result = db.prepare(
    "UPDATE orders SET status = 'paid' WHERE display_number = ? AND order_date = ?"
